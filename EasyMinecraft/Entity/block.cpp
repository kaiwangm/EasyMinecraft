#include "block.h"



block::block()
{
}

block::block(glm::vec3 pos, std::string na, Camera* ca,Shader* sha)
{
	
	Position = pos;
	name = na;
	camera = ca;
	shader = sha;
	ID = shader->getID();

	glGenBuffers(1, VBO);
	glBindBuffer(GL_ARRAY_BUFFER, VBO[0]);
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STREAM_DRAW);


	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)0); //Î»ÖÃ
	glEnableVertexAttribArray(0);
	glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)(3 * sizeof(float))); //ÑÕÉ«
	glEnableVertexAttribArray(2);

	
	glGenTextures(2, TexBuffer);

	glActiveTexture(GL_TEXTURE0);
	std::string file = "./Texture/blocks/gress.png";
	MakeTextureBuffer(TexBuffer[0], file.data());

	glActiveTexture(GL_TEXTURE1);
	MakeTextureBuffer(TexBuffer[1], "./Texture/blocks/grass_top.png");

	proMat = glm::perspective(glm::radians(45.0f), 1920.0f / 1080.0f, 0.1f, 100.0f);
	modelMat = glm::translate(glm::mat4(1.0f), Position);


}

block::~block()
{
}

void block::init()
{
	
}

void block::Draw()
{
	viewMat = camera->GetViewMatrix();
	

	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, TexBuffer[0]);
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, TexBuffer[1]);


	shader->use();
	glUniform1i(glGetUniformLocation(ID, "ourTexture"), 0);
	glUniform1i(glGetUniformLocation(ID, "faceTexCoord"), 1);
	glUniformMatrix4fv(glGetUniformLocation(ID, "viewMat"), 1, GL_FALSE, glm::value_ptr(viewMat));
	glUniformMatrix4fv(glGetUniformLocation(ID, "proMat"), 1, GL_FALSE, glm::value_ptr(proMat));

	glUniformMatrix4fv(glGetUniformLocation(shader->getID(), "modelMat"), 1, GL_FALSE, glm::value_ptr(modelMat));
	glDrawArrays(GL_TRIANGLES, 0, 36);
}
